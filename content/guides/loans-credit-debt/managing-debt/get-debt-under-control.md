---
title: "Get debt under control"
description: "List debts, prioritise, and pick a repayment method."
pillar: loans-credit-debt
hub: managing-debt
slug: get-debt-under-control
updated: "2026-08-03"
tags: 
  - managing debt
relatedTools: 
  - credit-card
  - personal-loan
relatedGuides: 
  - loans-credit-debt/managing-debt/urgent-help-with-money
  - loans-credit-debt/managing-debt/financial-hardship
  - loans-credit-debt/managing-debt/financial-counselling
  - loans-credit-debt/managing-debt/free-legal-advice
---

List debts, prioritise, and pick a repayment method. If money stress is affecting essentials this week, start at [Urgent help](/urgent). Debt stress shrinks options. Prioritise essentials, talk early, and use free help before high-cost credit.

## Get debt under control: the clear version

**Get debt under control** is worth understanding because small wording differences—fees, waiting periods, authorities, or exclusions—change outcomes. List debts, prioritise, and pick a repayment method. Debt stress shrinks options. Prioritise essentials, talk early, and use free help before high-cost credit.

Treat this page as a decision aid: gather your numbers, compare options, and keep written confirmations. Thresholds and eligibility can change, so verify critical figures with official sources or your provider before you act.

In Australia, providers must give key information, but it is still your job to check whether the product or process fits your cash flow. If a brochure uses jargon, rewrite it in one plain sentence you could explain to a friend.

## How to think about the moving parts

When **get debt under control** collides with debt stress, order of operations matters: essentials first, then contact lenders, then free counselling if you cannot see a path. Silence usually makes options worse.

People often skip the boring middle: documents, dates, and who else is affected. Write those down before you compare shiny features. If two options look identical, the tie-breaker is usually fees, exit flexibility, or how painful a mistake would be to reverse.

ClearMoney calculators that pair with this topic: [Credit Card](/tools/credit-card), [Personal Loan](/tools/personal-loan). Use them to turn guesses into numbers before you commit.

## Upsides worth wanting

- Hardship programs that can pause or reshape repayments
- Reduced stress when there is a written plan
- Protection pathways if collectors breach the rules

## Risks and traps to watch

### Easy-to-miss costs

Ignoring contact until default listing or legal steps begin. For **Get debt under control**, read the conditions that turn a good deal average.

### Timing and people problems

Using payday-style credit to patch structural shortfalls. Put review dates in your calendar.

### Recovery friction

Signing payment plans you cannot keep. Keep PDFs and screenshots; verbal promises are hard to enforce later.

## Details that change the decision

Collectors and lenders have rules; you have rights to request hardship consideration and to be treated fairly. For **Get debt under control**, keep a call log and prefer written agreements. Free financial counsellors help you prioritise and negotiate—use them before selling essentials or borrowing more to patch interest.

Also ask: what would make you reverse this choice in six months, and how expensive would that reverse be? Keep a one-page note with today’s balances, rates, and due dates so you are not reconstructing history from memory later.

If you only have fifteen minutes, spend them on the constraint that hurts most when ignored—usually cash timing, a fee, or a person who shares access—not on reading every marketing page.

## A short story: when timing changed the outcome

Luca avoided lender calls until fees stacked. Morgan asked for hardship in writing, listed essentials first, and booked a free counsellor session the same week. The lesson for **get debt under control**: slow clarity beats fast regret.

## Practical steps for Get debt under control

1. List debts with balances, rates, and minimums; mark essentials separately.
2. Contact lenders early and ask about hardship options in writing.
3. Speak with a free financial counsellor if repayments no longer fit.
4. Avoid high-cost short-term credit as a ‘bridge’ without a repayment date.

If you feel pressured, pause. A decision that cannot survive overnight scrutiny rarely deserves your signature.

## When life changes

- **Money stress now** — prioritise rent, food, utilities, and medicine, then use [Urgent help](/urgent). National Debt Helpline: 1800 007 007.

## What to do next

- Decide the next action for **get debt under control** in the next 48 hours (one call, one comparison, or one cancellation).
- Read [Urgent help with money](/topics/loans-credit-debt/managing-debt/urgent-help-with-money) for a related decision.
- Read [Financial hardship](/topics/loans-credit-debt/managing-debt/financial-hardship) for a related decision.
- Run the numbers with [Credit Card](/tools/credit-card).
- If essentials are at risk, open [Urgent help](/urgent) before taking on new credit.
